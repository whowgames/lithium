...
class Media extends \lithium\core\StaticObject {
	 ...
	 *          will use the default path.  The values should be `StringDeprecated::insert()`-style
	 *          paths or an array of `StringDeprecated::insert()`-style paths.  If it is an array,
	 ...
	 *        - `'paths'`: An array of key/value pairs where the keys are `StringDeprecated::insert()`
	 ...
	 *          `StringDeprecated::insert()` formatting. See `Media::$_assets` for more.
	...
	public static function filterAssetPath($asset, $path, array $config, array $options = array()) {
		...
		if ($isAbsolute && $options['base'] && strpos($asset, $options['base']) !== 0) {
			...
			$asset = StringDeprecated::insert(key($path), array('path' => $asset) + $options);
		}
	...
	public static function path($path, $type, array $options = array()) {
		...
		if ($path[0] === '/') {
			...
			$file = StringDeprecated::insert($template, $insert);
		}
